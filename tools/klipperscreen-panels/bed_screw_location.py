...
class Panel(ScreenPanel):
    ...
    def load_generated_points_by_name(self):
        ...
        named_points = [
            {"name": names[index], "x": x, "y": y}
            for index, (x, y) in coordinates.items()
            if names.get(index)
        ]
        if len(named_points) == 7:
            named_points = [point for point in named_points if point["name"] != "Center"]
        physical_count = len(named_points)
        ...
        for key, _column, _row, visual_label in self.slot_layout():
            # Schema 5 was the short-lived mirrored release. Schema 6 already
            # used physical names directly, so rotating it would corrupt valid
            # user-taught locations.
            source_label = mirrored_migration[visual_label] if generated_schema == 5 else visual_label
    ...
    def orientation(self):
        # Bed screw controls always represent the physical printer as viewed
        # by an operator standing at its front edge. Display/touch rotation and
        # KlipperScreen axis inversion must never transform machine targets.
