...
class Panel(ScreenPanel):
    ...
    def load_generated_points_by_name(self):
        ...
        mapped = {}
        # The stock generated files name screw positions from the rear of the
        # machine. The panel is deliberately drawn from the operator/front
        # edge, so rotate only those imported labels by 180 degrees. Screen,
        # touch, arrow, and manually taught coordinate orientation is separate.
        for key, _column, _row, visual_label in self.slot_layout():
            source_label = visual_label if generated_schema >= STATE_VERSION else opposite_name[visual_label]
