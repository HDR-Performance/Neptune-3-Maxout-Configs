...
class Panel(ScreenPanel):
    ...
    def load_state(self):
        try:
            ...
            self.points = {}
        if self.count == 0:
            self.load_generated_points()

    def load_generated_points(self):
        try:
            text = open(GENERATED_FILE, encoding="utf-8").read()
        except OSError:
            return
        found = []
        for match in re.finditer(r"(?m)^screw(\d+):\s*([-+]?[0-9.]+)\s*,\s*([-+]?[0-9.]+)\s*$", text):
            found.append((int(match.group(1)), float(match.group(2)), float(match.group(3))))
        found.sort()
        # The Max baseline contains a center reference plus six physical
        # adjusters. The setup UI edits only the actual adjusters.
        if len(found) == 7:
            found = found[1:]
        if len(found) not in (4, 5, 6):
            return
        self.count = len(found)
        self.points = {
            str(index): {"x": x, "y": y}
            for index, (_source_index, x, y) in enumerate(found, start=1)
        }

    def save_state(self):
        os.makedirs(os.path.dirname(STATE_FILE), exist_ok=True)
        payload = {"version": 1, "count": self.count, "points": self.points}
    ...
    def suggested_point(self, column, row):
        ...
        margin_x = max(10.0, (xmax - xmin) * 0.12)
        margin_y = max(10.0, (ymax - ymin) * 0.12)
        x_positions = [xmin + margin_x, (xmin + xmax) / 2.0, xmax - margin_x]
        # Map row zero is visually rear/high Y; row two is front/low Y.
        y_positions = [ymax - margin_y, (ymin + ymax) / 2.0, ymin + margin_y]
        return x_positions[column], y_positions[row]
    ...
    def show_jog(self, name):
        ...
        controls = {
            "left": ("arrow-left", "X", "-", 0, 2),
            "right": ("arrow-right", "X", "+", 2, 2),
            "up": ("arrow-up", "Y", "+", 1, 1),
            "down": ("arrow-down", "Y", "-", 1, 3),
        }
        for icon, axis, direction, column, row in controls.values():
            button = self._gtk.Button(icon, None, "color1")
            button.connect("clicked", self.jog, axis, direction)
    ...
    def jog(self, widget, axis, direction):
        if self.printer_busy():
            self.popup(_("Movement is blocked while printing or paused."), 2)
            return
        script = f"G91\nG0 {axis}{direction}{self.distance} F3000\nG90"
        self._screen._send_action(widget, "printer.gcode.script", {"script": script})
