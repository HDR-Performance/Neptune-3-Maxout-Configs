...
[[ -d "${CONFIG_DIR}" && -f "${CONFIG_DIR}/printer.cfg" ]] || die "Current config directory is invalid: ${CONFIG_DIR}"
...
[[ -f "${SOURCE_BACKUP}/printer.cfg" ]] || die "Selected backup does not contain printer.cfg: ${SOURCE_BACKUP}"
...
[[ -f "${SAFETY_BACKUP}/printer.cfg" ]] || die "Safety backup failed; restore stopped."
